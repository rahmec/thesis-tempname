...
def generate_matrix(v):
	M = Matrix(GF(2), r, n, lambda i, j: random.choices([0, 1], weights=[v,1-v], k=1)[0])
	...
	for comb11 in combs11:
		...
		for comb12 in combs12:
			...
			if x11*A1.T == -x12*A2.T:
				logging.debug("Found collision")
				x1 = vector(GF(2), list(x11) + list(x12))
				x2 = -x1*B.T
				x = vector(GF(2), list(x1) + list(x2))
				w = x.hamming_weight()
