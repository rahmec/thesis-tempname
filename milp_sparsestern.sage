...
def choose_l1(H):
	...
	# Decision variables: xi for rows, zj for columns
	x = p.new_variable(binary=True)
	z = p.new_variable(binary=True)

	# Objective function: Minimize the sum of activated columns zj
	p.set_objective(sum(z[j] for j in range(n)))

	# Constraints:
	# Constraint 1: Ensure that for each j, zj >= aij * xi
	for j in range(n):
		for i in range(r):
			p.add_constraint(z[j] >= H[i][j] * x[i])

	# Constraint 2: Exactly l rows must be selected
	p.add_constraint(sum(x[i] for i in range(r)) == l1)

	# Solve the problem
	p.solve()

	# Get the results
	selected_rows = [i for i in range(r) if p.get_values(x[i]) > 0.5]
	activated_columns = [j for j in range(n) if p.get_values(z[j]) > 0.5]

	print("Selected rows:", selected_rows)
	print("Activated columns:", len(activated_columns))
